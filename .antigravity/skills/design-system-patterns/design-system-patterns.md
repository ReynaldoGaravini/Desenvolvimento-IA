...
<pre>`// Design tokens with CSS custom properties
const tokens = {
  colors: {
    // Primitive tokens
    gray: {
...
<pre>`/* Layer 1: Primitive tokens (raw values) */
:root {
...
<pre>`import { createContext, useContext, useEffect, useState } from "react";
...
<pre>`import { cva, type VariantProps } from "class-variance-authority";
